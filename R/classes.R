...
#' Internal constructor for clinprog signature object
#'
#' @description Creates a standardized `clinprog_signature` S3 object from a signature table. This function ensures that
#' the input signature is valid, properly formatted, and consistent across clinprog modules.
#'
#' @details This constructor is used internally by clinprog functions (e.g., \code{run_regression()}
#' and \code{run_survival()}) to guarantee object integrity and standardization across clinprog modules.
#'
...
#'
#' @return
#' An object of class \code{"clinprog_signature"} containing:
#' \itemize{
...
new_clinprog_signature <- function(signature, plots = NULL, params = NULL, metadata = NULL, call = NULL) {
  ...
  obj <- structure(
    ...
    class = "clinprog_signature"
  )

  return(obj)
}

#' Internal constructor for clinprog survival object
#'
#' @description Creates a standardized `clinprog_survival` S3 object from clinprog survival analysis outputs.
#' This function ensures that the generated object is structurally valid and internally consistent.
#'
#' @details This constructor is used internally by \code{run_survival()} to guarantee object integrity
#' and standardization across clinprog modules.
#'
#' It performs the following steps:
#' \itemize{
...
#'
#' @param signature A data.frame containing the clinprog molecular signature with at least two columns:
#'   \itemize{
#'     \item \code{feature}: Feature names (e.g., genes or transcripts)
#'     \item \code{coefficient}: Numeric coefficients associated with each feature
#'   }
#' @param score_cutoff Numeric cutoff used to define low/high clinprog score groups.
...
#'
#' @return
#' An object of class \code{"clinprog_survival"} containing:
#' \itemize{
#'   \item \code{signature}: clinprog molecular signature
...
#'
#' @keywords internal
new_clinprog_survival <- function(signature, score_cutoff, expression_data, survival, ph_result, clinical_data = NULL,
  ...
  obj <- structure(
    ...
    class = "clinprog_survival"
  )

  return(obj)
}

#' Create a clinprog complete analysis object
#'
#' @description Internal constructor for objects of class `clinprog_complete`.
#'
#' @param regression Object of class `clinprog_signature` generated by \code{run_regression()}.
#' @param survival Object of class `clinprog_survival` generated by \code{run_survival()}.
...
#'
#' @details Validates and creates a standardized \code{"clinprog_complete"} object used internally by the package.
#'
#' @return An object of class \code{"clinprog_complete"}.
#'
#' @keywords internal
